...
export default function GetUsers({ data }) {
      ...
      <ul>
        {data.map(item => (
          <li key={item.id}>
            ...
            <Link href={"/studentreg/dashboard/courseuser/" + item.id}><div className="font-bold text-blue-600/100 bg-gray-200 px-16 m-10"
            >{item.Coursename}</div></Link>
            </li>
        ))}
      </ul>
        </div>
 ...
 export async function getServerSideProps() {
      ...
      const response = await axios.get('http://localhost:3000/reg/indexcourse');
